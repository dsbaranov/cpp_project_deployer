      ...
      }
      {
        ...
        if (log_duration_file.is_open())
        {
          log_duration_file
              ...
              << "{}\n"
              << "~LogDuration()\n"
              << "{using namespace std::chrono; using namespace std::literals;const auto end_time = Clock::now();const auto dur = end_time - start_time_;std::cerr << id_ << \" : \"s << duration_cast<milliseconds>(dur).count() << \" ms\"s << std::endl;}\n"
              << "private:\n"
